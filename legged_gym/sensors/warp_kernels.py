...
@wp.kernel(enable_backward=False)
def raycast_multi_env_kernel(
    meshes: wp.array(dtype=wp.uint64),      # 每个 env 的 mesh 句柄 (len = num_envs)
    num_rays_per_env: int,                  # 每个环境的射线数
    ray_starts: wp.array(dtype=wp.vec3),    # 所有 env 展平后的 ray 起点 [num_envs*num_rays, 3]
    ray_dirs: wp.array(dtype=wp.vec3),      # 所有 env 展平后的 ray 方向 [num_envs*num_rays, 3]
    ray_hits: wp.array(dtype=wp.vec3),      # 输出：命中点 [num_envs*num_rays, 3]
    ray_distance: wp.array(dtype=wp.float32), # 输出：命中距离 [num_envs*num_rays]
    ray_normal: wp.array(dtype=wp.vec3),    # 输出：命中法向 [num_envs*num_rays, 3]
    ray_face_id: wp.array(dtype=wp.int32),  # 输出：命中面索引 [num_envs*num_rays]
    max_dist: float = 1e6,
    return_distance: int = 0,
    return_normal: int = 0,
    return_face_id: int = 0,
):
    """Batch raycasting kernel — each environment has its own mesh."""
    tid = wp.tid()

    env_id = tid // num_rays_per_env   # 当前线程对应的环境索引
    mesh = meshes[env_id]              # 当前环境对应的 mesh
    ...
    # 执行射线查询
    hit = wp.mesh_query_ray(mesh, ray_starts[tid], ray_dirs[tid], max_dist, t, u, v, sign, n, f)

    if hit:
        ray_hits[tid] = ray_starts[tid] + t * ray_dirs[tid]
